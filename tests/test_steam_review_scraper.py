...
from steam_review_scraper import get_reviews, flatten_review, main
...
async def test_get_reviews_success(mock_session):
    ...
    mock_response.json = AsyncMock(return_value={"success": 1, "reviews": [{"review_id": 1, "review": "good"}], "cursor": "next_cursor"})
...
async def test_get_reviews_rate_limit(mock_session):
    ...
    mock_response_success.json.return_value = {"success": 1, "reviews": [], "cursor": "next_cursor"}
    ...
    with patch('asyncio.sleep', new=AsyncMock()) as mock_sleep:
        reviews_data = await get_reviews(mock_session, app_id, retries=1, backoff_factor=0)
...
async def test_main_function_csv_writing(tmp_path):
    mock_reviews_data = {
        "success": 1,
        "reviews": [
            {"review_id": 1, "review": "Test 1", "author": {"steamid": "1"}, "voted_up": True},
            {"review_id": 2, "review": "Test 2", "author": {"steamid": "2"}, "voted_up": False},
        ],
        "cursor": "next_cursor"
    }
    ...
    with patch('steam_review_scraper.get_reviews', new=AsyncMock(return_value=mock_reviews_data)) as mock_get_reviews, \
         patch('steam_review_scraper.get_app_details', new=AsyncMock(return_value=mock_app_name)) as mock_get_app_details:
        ...
        mock_session_get.__aenter__.return_value = mock_session_get # Return self for async with
        mock_session_get.__aexit__.return_value = None # No specific exit behavior needed
        
        with patch('aiohttp.ClientSession', return_value=mock_session_get) as mock_client_session_constructor:
            ...
            def mock_open(file, mode='r', buffering=-1, encoding=None, errors=None, newline=None, closefd=True):
                if isinstance(file, (str, Path)) and str(file).endswith(expected_filename):
                    return original_open(output_csv_path, mode, buffering, encoding, errors, newline, closefd)
                return original_open(file, mode, buffering, encoding, errors, newline, closefd)
...
import builtins # Import builtins for test_main_function_csv_writing


import csv # Import csv for test_main_function_csv_writing
