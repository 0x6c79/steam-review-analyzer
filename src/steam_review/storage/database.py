...
class ReviewDatabase:
    ...
    def _init_db(self):
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute('CREATE INDEX IF NOT EXISTS idx_detected_language ON reviews(detected_language)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_playtime ON reviews(playtime_forever)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_sentiment ON reviews(sentiment_score)')
            # Composite indexes for common queries
            conn.execute('CREATE INDEX IF NOT EXISTS idx_app_timestamp ON reviews(app_id, timestamp_created)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_app_voted ON reviews(app_id, voted_up)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_app_language ON reviews(app_id, language)')
    ...
    def get_reviews(self, app_id=None, limit=None, start_date=None, end_date=None, 
                   min_playtime=None, max_playtime=None, language=None, 
                   sentiment_min=None, sentiment_max=None, voted_up_only=None):
        """
    ...
    def get_stats(self, app_id=None, start_date=None, end_date=None):
        ...
        query = 'SELECT COUNT(*) as total, SUM(voted_up) as positive FROM reviews WHERE 1=1'
        params = []
        if app_id:
            query += ' AND app_id = ?'
            params.append(str(app_id))
    ...
    def get_date_range(self, app_id=None):
        ...
        if not df.empty:
            row = df.iloc[0]
            return {
                'min': pd.Timestamp(row['min_date'], unit='s') if row['min_date'] else None,
                'max': pd.Timestamp(row['max_date'], unit='s') if row['max_date'] else None
            }
