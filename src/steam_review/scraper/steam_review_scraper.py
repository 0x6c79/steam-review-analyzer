        ...
        try:
            ...
            async with session.get(url, timeout=DEFAULT_TIMEOUT) as response:
                if response.status == 429:
                    ...
                    logging.warning(
                        f"Rate limit hit (429). Waiting {wait_time} seconds..."
                    )
...
def load_checkpoint(app_id: str) -> tuple[str, int]:
    ...
    if os.path.exists(checkpoint_file):
        try:
            ...
            logging.info(
                f"Resuming from checkpoint: cursor={cursor[:50]}..., total={total}"
            )
            return cursor, total
        except:
            pass
...
async def main(app_id, limit=0, incremental=False):
    ...
    LIMIT = (
        ...
        else (DEFAULT_INCREMENTAL_LIMIT if incremental else 0)
    )
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            if game_name:
                ...
                logging.warning(
                    f"Could not retrieve game name for App ID {app_id}. Using default filename: {output_filename}"
                )
            ...
            if os.path.exists(output_filename):
                ...
                if existing_ids:
                    logging.info(
                        f"Found {len(existing_ids)} existing reviews in {output_filename}"
                    )
            ...
            with open(
                output_filename, write_mode, newline="", encoding="utf-8"
            ) as csvfile:
                writer = None
                new_reviews_count = 0
                ...
                while True:
                    ...
                    if reviews_data and reviews_data.get("success") == 1:
                        ...
                        flattened_reviews = [
                            flatten_review(review) for review in reviews
                        ]

                        for r in flattened_reviews:
                            rid = r.get("recommendation_id") or r.get(
                                "recommendationid"
                            )
                        ...
                        new_reviews = [
                            r
                            for r in flattened_reviews
                            if r.get("recommendation_id") not in existing_ids
                        ]
                        ...
                        if not new_cursor or new_cursor == cursor:
                            logging.info(
                                "No more reviews available (cursor unchanged)."
                            )
                            break
                        cursor = new_cursor
                        ...
                        if LIMIT > 0 and total_reviews_fetched >= LIMIT:
                            logging.info(
                                f"Reached the limit of {LIMIT} reviews. Stopping."
                            )
                            break
                        ...
                        if consecutive_failures >= 5:
                            logging.error(
                                "Failed to fetch reviews 5 times consecutively. Saving checkpoint and exiting."
                            )
                            save_checkpoint(app_id, cursor, total_reviews_fetched)
                            break
                            logging.warning(
            ...
            logging.info(
                f"Successfully saved {new_reviews_count} new reviews ({mode_str} mode) to {output_filename}"
            )
            ...
            try:
                ...
                if "voted_up" in df.columns:
                    if df["voted_up"].dtype == "bool":
                        ...
                        vote_map = {"True": 1, "False": 0, True: 1, False: 0}
                        df["voted_up"] = (
                            df["voted_up"].map(vote_map).fillna(0).astype(int)
                        )
                ...
                logging.info(
                    f"📈 Updated database stats - Total: {db_stats['total']}, Positive: {db_stats['positive']}, Negative: {db_stats['negative']}"
                )

            except Exception as e:
                logging.warning(
                    f"Auto-import failed: {e}. You can manually import later with: python -m src.steam_review.storage.auto_import"
                )
        ...
        if "cursor" in locals() and "total_reviews_fetched" in locals():
            save_checkpoint(app_id, cursor, total_reviews_fetched)
            logging.info(
                f"Checkpoint saved. Run again to resume from {total_reviews_fetched} reviews."
            )
    except IOError as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Scrape Steam reviews for a given App ID."
    )
    parser.add_argument(
        "--app_id",
        type=str,
